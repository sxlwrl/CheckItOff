...
class UserModel {
    ...
    async getUser(data){
        return new Promise((resolve, reject) => {
            this.database.query('SELECT * FROM users WHERE username=? OR email=?', [data, data], (error, result) => {
    ...
    checkExistingUser = (username, email) => {
        return new Promise((resolve, reject) => {
            this.database.query('SELECT username, email FROM users WHERE username=? OR email=?', [username, email], (error, result) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    registerUser = (username, email, hashedPassword) => {
        return new Promise((resolve, reject) => {
            this.database.query('INSERT INTO users SET ?', {username: username, email: email, pass: hashedPassword}, (error, result) => {
